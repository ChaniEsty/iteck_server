...
class JobsController {
     getJobs=async(req,res) => {
        const jobList=jobsDal.getJobs(req.body)
     ...
     createJob=async(req,res) => {
         const job=jobsDal.createJob(req.body);
        job=="New job created"? res.status(201).json({ message: 'New job created' }): res.status(400).json({ message: 'Invalid job data received' })
